...
```yaml
annotations:
  runbook_url: https://nerd.dad/latest/homelab/kubernetes/.../mk_runbook_.../
...
Runbooks are MkDocs pages under `docs/homelab/kubernetes/my-apps/observability/runbooks/` in the **nerd.dad** repo. ntfy **Runbook** buttons use `runbook_url` on the PrometheusRule.

**Freshness:** pages use Git revision dates on GitHub Pages. If ntfy opens a 404, redeploy the nerd.dad site (`mike`) so `latest` includes your commit.

1. Copy `docs/homelab/kubernetes/my-apps/observability/runbooks/mk_runbook_template.md` → `mk_runbook_<alert-kebab>.md` in the same folder
2. Set `alertname` and optional `alertnames` in front matter; update the runbook index if you maintain it by hand
3. Set `runbook_url` on the alert in truecharts to the Pages URL, e.g. `https://nerd.dad/latest/homelab/kubernetes/my-apps/observability/runbooks/mk_runbook_<alert-kebab>/`
4. Commit here and deploy Pages; commit the PrometheusRule in truecharts
...
See **`docs/homelab/kubernetes/my-apps/observability/alert-test/mk_alert-test.md`**. A deliberate `alert-test-fail` HelmRelease (nonexistent chart) plus `HomelabFluxHelmReleaseTestFail` (2m `for`) lets you verify ntfy without waiting 10 minutes. Remove `alert-test/` and `homelab-flux-test.yaml` (in truecharts) when done.
...
| Your own SLOs | New `prometheus-rules/app/homelab-<team>.yaml` (truecharts) + runbook under `docs/homelab/kubernetes/my-apps/observability/runbooks/` |
